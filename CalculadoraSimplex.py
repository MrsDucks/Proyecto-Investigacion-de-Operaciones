Funcion_Z = (int(input("Digite el numero de variables en Z: ")))
num_restricciones = (int(input("Digite el numero de las Restricciones: ")))
num_filas = num_restricciones + 1
num_colum = num_restricciones + Funcion_Z + 2
matriz_1 = []
matriz_2 = []
lista = []
respuestas = {}
...
for i in range(num_filas):
    for j in range(num_colum):
        if j == 0 and i != num_filas - 1:
            ...
            matriz_1[i][j] = 1
        elif 0 < j <= Funcion_Z and i != num_filas - 1:
            matriz_1[i][j] = int(
